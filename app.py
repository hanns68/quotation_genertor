...
with st.sidebar:
    ...
    tax_type = st.radio("稅務設定", ["未稅金額", "含稅金額"])
...
with col4:
    ...
    if st.button("➕ 新增項目"):
        if item_n:
            # 關鍵：直接存入字典 (Dictionary)，避免 TypeError
...
if st.session_state.items:
    ...
    with c_btn2:
        # 下載 PDF 按鈕
        info_payload = {"title": title, "company": company, "tax_id": tax_id, "phone": phone, "date": date}
    ...
    st.info("請於上方輸入資料並點擊『新增項目』開始建立清單。")
